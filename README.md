...
A Windows Rust/egui controller with a Python USB bridge and Lua scenes for HoloCubic.
...
需要 Windows 10/11 x64、Python 3.8+，以及匹配本项目 Lua 接口的 HoloCubic/ESP32-S3 设备。电脑必须能够访问设备串口。
...
保留 EXE 同目录的 Python 脚本、相邻 `holo_usb_display.py` 和 `assets/`；这不是单 EXE 程序。`python` 必须在 PATH 中。
...
根目录是 Cargo workspace，Rust 源码仍位于 `rs_holocubic/`。双击本机构建的 `target/release/rs_holocubic.exe` 也可以运行；对外分发请使用 `pwsh -File scripts/package.ps1` 生成包含脚本的完整包。
...
GitHub CI 运行 Rust/Python 自动测试并构建完整 Windows 包，不连接真实设备、不读取个人日历或账号。Lua 场景和真实硬件仍需手动验证；历史记录和各模式细节见 [控制台说明](rs_holocubic/README.md)。
